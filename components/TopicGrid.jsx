            ...
            if (topic.id === 'choices') {
              ...
              return (
                <div key={topic.id} className="text-center">
                  ...
                  <div className="bg-slate-700/30 rounded-lg p-4 mb-4">
                    <div className="grid grid-cols-2 gap-2 text-xs">
                      <div className="text-red-400">
                        <span className="font-bold">الأحمر:</span>
                        <div className="mt-1 max-h-24 overflow-y-auto">
                          {teamCountries && teamCountries.red && teamCountries.red.length > 0 ? (
                            teamCountries.red.map(countryId => {
                              const country = topic.countries.find(c => c.id === countryId);
                              return country ? (
                                <div key={countryId} className="bg-red-600/50 rounded px-1 mb-1 text-xs">
                                  {country.flag} {country.name}
                                </div>
                              ) : null;
                            })
                          ) : (
                            <div className="text-slate-500">لا توجد دول</div>
                          )}
                        </div>
                      </div>
                      
                      <div className="text-blue-400">
                        <span className="font-bold">الأزرق:</span>
                        <div className="mt-1 max-h-24 overflow-y-auto">
                          {teamCountries && teamCountries.blue && teamCountries.blue.length > 0 ? (
                            teamCountries.blue.map(countryId => {
                              const country = topic.countries.find(c => c.id === countryId);
                              return country ? (
                                <div key={countryId} className="bg-blue-600/50 rounded px-1 mb-1 text-xs">
                                  {country.flag} {country.name}
                                </div>
                              ) : null;
                            })
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div className="space-y-2">
                      <div className="text-xs font-bold text-red-400 mb-1">نقاط الأحمر</div>
                      <div className={`w-full p-2 md:p-3 text-xs md:text-sm rounded-lg font-bold transition-all duration-300 border-2 ${
                    ...
                    <div className="space-y-2">
                      <div className="text-xs font-bold text-blue-400 mb-1">نقاط الأزرق</div>
                      <div className={`w-full p-2 md:p-3 text-xs md:text-sm rounded-lg font-bold transition-all duration-300 border-2 ${
                        'bg-gradient-to-br from-blue-500 to-indigo-500 text-white border-blue-400'
                      }`}>
                  ...
                  <div className="mt-2 text-xs text-slate-400">
                    {currentTurn === 'red' ? '🔴' : '🔵'} اختر دولة من الخريطة أعلاه
                  </div>
                </div>
              );
            } else {
